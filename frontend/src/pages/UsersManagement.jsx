...
const UsersManagement = () => {
    ...
    const [filteredUsers, setFilteredUsers] = useState(data?.users.data || []);
    ...
    const handleSuspendSubmit = () => {
        const updatedUsers = data.users.map(user =>
            user.id === suspendModal.user.id ? { ...user, status: 'SUSPENDED' } : user
        );
        setData({ ...data, users: updatedUsers });
        setSuspendModal({ open: false, user: null });
        setSuspendReason('');
        // In a real app, suspendReason would be sent to a backend
    };
